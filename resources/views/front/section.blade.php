@extends('layouts.main-front',[
        ...
        'sl'=> false,
        'sub'=>!empty($section->excerpt) ?$section->excerpt :\Illuminate\Support\Str::limit(trim(preg_replace('/\s\s+/', ' ', $section->description)),150),
        'subLink'=>'',
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                        ...
                        <div class="col-lg-9">
                            <div class="chaleshkade-post-top">
                                <a class="author-avatar"
                                   href="{{$section->type == 1 ? 'javascript:void(0)' : route('user',$section->user->username)}}">
                                    @if($section->type ==1)
                                ...
                                <div class="chaleshkade-post-author">
                                    ...
                                    <div class="chaleshkade-author-meta mt-2">
                                        <div class="author-badge">
                                            ...
                                            <span><a
                                                    href="{{route('category',$section->category->title)}}">{{$section->category->title}}</a></span>
                                        </div>
                                        <div class="author-badge">
                                            <i class="icon_calendar"></i>
                                            <a href="javascript:void(0)" style="float: left" class="mr-1">مهلت تا
                    ...
                    <hr>
                @if($section->status == 2 || $section->status == 4)
                    <!-- chaleshkade post content -->
                        <div class="q-title mb-3">
                            <span class="question-icon"
                                  title="Question">{{$section->kind == 0 ? "چالش " : "سوال "}}:</span>
                            <h1>{{$section->title}}</h1>
                            @if($section->kind == 0)
                                <span
                                    class="badge prize_text">جایزه چالش: {{$section->type == 1 ? $section->prize_text: "امتیاز توسط کاربر"}}</span>
                            @endif
                        </div>
                        <div class="chaleshkade-post-content">
                            <div class="content">
                                {!! $section->description !!}
                                <form action="{{route('quiz',$section)}}" id="quiz" method="post" class="">
                                    @csrf
                                            ...
                                            @forelse($question->answers as $key=> $answer)
                                                ...
                                                <div class="col-sm-6 form-group pb-5 ansy"
                                                    ...
                                                    <div id="registerInputWrapper">
                                                        <div class="form-check aa">
                                                            {{--                                                            <input type="hidden" name="answer[{{ $question->id }}]" value="">--}}
                                                            <input class="form-check-input" style="right: 0px"
                                                                   type="radio"
                                                                   ...
                                                                   @if(old("answer." . $question->id) == $answer->id )     checked="checked"
                                                                   @endif
                                                                   value="{{$answer->id}}">
                                                            <label class="form-check-label mr-4"
                                                                ...
                                                                @if($section->status == 4 && $answer->is_checked == 1 )
                                                                    <span
                                                                        class="badge badge-success">گزینه صحیح: </span>
                                        ...
                                        @else
                                            ...
                                            @if(auth()->check())
                                                <div class="col-lg-12 text-center">
                                                    @if($section->type != 1 && $section->user_id == auth()->id())
                                                        <div class="alert alert-info">شما نمیتوانید در چالش خود شرکت
                                                            کنید
                                                            !
                                                        </div>
                                                    @else
                                                        @if($section->hasDone(auth()->id()))
                                                            <div class="alert alert-success">با تشکر از شرکت شما در
                                                                آزمون
                                                            </div>
                                            ...
                                            @else
                                                <div class="alert alert-warning text-center">
                                                    <a href="{{route('login')}}"> جهت شرکت در چالش باید ثبت نام کنید یا
                                                        وارد
                                                        شوید.</a>
                                                </div>
                                            @endif
                                    ...
                                    <div class="best-answer">
                                        <div class="row">
                                            ...
                                            <div class="col-sm-10 mb-12" style="background: #eaff62;padding: 10px;">
                                                ...
                                                <p class="bst-p">
                                                    برنده این چالش <a
                                                        href="{{route('user',$best_user->user->username)}}">{{$best_user->user->name}}</a>
                                                    است با
                                                    کسب {{$best_user->score}}
                                                    امتیاز !
                                                </p>
                            ...
                            @else
                                <div class="alert alert-info text-center">جهت کامنت دادن باید وارد شوید یا ثبت نام
                                    کنید.
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="all-answers" id="replies">
                            ...
                            @if($replies->count() == 0)
                                <div class="alert alert-info text-center">در حال حاضر کامنتی وجود ندارد.</div>
                            @endif
                            @forelse($replies as $reply)
                                @if($reply->status == 1 || $reply->user_id == auth()->id())
                                                ...
                                                <div class="chaleshkade-author-meta">
                                                    ...
                                                    <div class="author-badge">
                                                        ...
                                                        <a href="javascript:void(0)" style="float: left"
                                                           class="mr-1">{{Verta::instance($reply->created_at)->formatDifference()}}</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            @if($reply->status == 1 && auth()->check())
                                                <div class="action-button-container action-btns">
                                                    <button type="button" class="action_btn btn-ans ask-btn reply-btn"
                                                            ...
                                                            href="#collapseReplyChild{{$reply->id}}">
                                                        پاسخ
                                                        به
                                                        این کامنت
                                                    </button>
                                                    ...
                                                    @if($reply->user_id != auth()->id())
                                                        @if( \App\Models\Like::query()->whereMorphedTo('userable', auth()->user())->where([['likeable_id',$reply->id],['is_liked', true]])->get()->count() > 0)
                                                            <form action="{{route('unlikeReply',$reply->id)}}"
                                                                  method="post">
                                                                @csrf
                                                                <button type="submit"
                                                        ...
                                                        @else
                                                            <form action="{{route('likeReply',$reply->id)}}"
                                                                  method="post">
                                                                @csrf
                                                                <button type="submit"
                                                        ...
                                                        <form action="{{route('dislikeReply',$reply->id)}}"
                                                              method="post">
                                                            @csrf
                                                            <button type="submit"
                                    ...
                                    <textarea name="body" class="form-control" rows="2"
                                              cols="2">{{old('body')}}</textarea>
                                                    ...
                                                    <div class="form-group m-b-0">
                                                        <button type="submit"
                                                                class="btn btn-success btn-rounded m-t-10">
                                                            ارسال
                                                        </button>
                                                    </div>
                                                </form>
                                ...
                                @forelse($reply->children as $replyChild)
                                    @if($replyChild->status == 1 || $replyChild->user_id == auth()->id())
                                        <div class="best-answer reprep">
                                            <div class="row">
                                                <div class="col-lg-9">
                                                    <div class="chaleshkade-post-top">
                                                        <a class="author-avatar"
                                                        ...
                                                        <div class="chaleshkade-post-author">
                                                            ...
                                                            <div class="chaleshkade-author-meta">
                                                                ...
                                                                <div class="author-badge">
                                                                    <i class="icon_calendar"></i>
                                                                    <a href="javascript:void(0)" style="float: left"
                                                                       class="mr-1">{{Verta::instance($replyChild->created_at)->formatDifference()}}</a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class=" d-flex">
                                                ...
                                                <p class="pt-1 pb-1 pr-3 pl-3">{!! $replyChild->body !!}</p>
                                                @if($replyChild->status == 1 && auth()->check())
                                                    <div class="action-button-container action-btns">
                                                        @if($replyChild->user_id != auth()->id())
                                                            @if( \App\Models\Like::query()->whereMorphedTo('userable', auth()->user())->where([['likeable_id',$replyChild->id],['is_liked', true]])->get()->count() > 0)
                                                                <form action="{{route('unlikeReply',$replyChild->id)}}"
                                                                      method="post">
                                                                    @csrf
                                                                    <button type="submit"
                                                                            class="btn btn-sm btn-warning  ml-2">
                                                            ...
                                                            @else
                                                                <form action="{{route('likeReply',$replyChild->id)}}"
                                                                      method="post">
                                                                    @csrf
                                                                    <button type="submit"
                                                                            class="btn btn-sm btn-success ml-2">
                    ...
                    @else
                        <div class="alert alert-info p-5 text-center">چالش در حال بررسی است.</div>
                    @endif
                </div>
                <!-- /.col-lg-8 -->
...
@section('scripts')
        <script>
            $('#quiz').on('submit', function() {
                if($('input:radio:checked').length < {{$section->questions->count()}}) {
                    event.preventDefault()

                    $.toast({
                        heading: 'خطا!'
                        , text: 'لطفا برای هر سوال، یک گزینه را انتخاب کنید.'
